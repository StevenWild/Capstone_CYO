...
We should have a look at the various attributes to see if there are any that we can remove that wont help us with our predictions.
Different types of plots were looked at , but the violin plot seems to highlight the attributes that will be most useful to assist with the predictions. We will be looking for those attributes where there are marked differences in the distributions.
...
Each method follows a similar pattern:
      ...
      * validate the predictions against the real outcom
...
We get an overall accuracy of 73%, much better than just guessing.
...
We do not know at this point which k parameter would give the lowest error 
...
prevent over/under training
regularisation to prevent over-fitting
could split into validation set and tune the hyper parameters
becasue we have very little data, comparatively speaking, we could use cross validation
